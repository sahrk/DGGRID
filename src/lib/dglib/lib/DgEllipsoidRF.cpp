...
////////////////////////////////////////////////////////////////////////////////
long double DgGeoCoord::gcDist(const DgGeoCoord &g1, const DgGeoCoord &g2,
                               bool rads)
/*
...
////////////////////////////////////////////////////////////////////////////////
DgGeoCoord DgGeoCoord::gcIntersect(const DgGeoCoord &g11, const DgGeoCoord &g12,
                                   const DgGeoCoord &g21,
...
////////////////////////////////////////////////////////////////////////////////
long double DgGeoCoord::geoTriArea(const DgGeoCoord &g1, const DgGeoCoord &g2,
                                   const DgGeoCoord &g3)
/*
...
////////////////////////////////////////////////////////////////////////////////
long double DgGeoCoord::geoPolyArea(const DgPolygon &poly,
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
const char *DgEllipsoidRF::str2add(DgGeoCoord *add, const char *str,
...
*/
{
  ...
  dgcout << "  A: ";
  printVal(tri.edges[0]);
  dgcout << "  B: ";
  printVal(tri.edges[1]);
  dgcout << "  C: ";
  printVal(tri.edges[2]);
  dgcout << "\n";

  dgcout << "  a: ";
  printVal(tri.angles[0] * M_180_PI);
  dgcout << "  b: ";
  printVal(tri.angles[1] * M_180_PI);
  dgcout << "  c: ";
  printVal(tri.angles[2] * M_180_PI);
  dgcout << "\n";

  dgcout << "  area: ";
  printVal(tri.area);
  dgcout << "  perimeter: ";
  printVal(tri.perimeter);
  dgcout << "  compactness: ";
  printVal(tri.compactness);
  dgcout << "\n";
...
long double sqrMetersToExcessD(long double area) {
  return area * 360.0L /
         (4.0L * M_PI * DgGeoSphRF::earthRadiusKM() *
...
/*
  ...
  longitude Input unit is degree. Output unit regard the earth's radius as one
  unit.
*/
{
  Vec3D v;
  ...
  if (fabsl(v.x) < epsilon)
    v.x = 0.0L;
  if (fabsl(v.y) < epsilon)
    v.y = 0.0L;
  if (fabsl(v.z) < epsilon)
    v.z = 0.0L;
...
*/
{
  ...
  if (fabsl(v.z) - 1.0L < PRECISION) {
    ...
    if (v.z < -1.0L)
      v.z = -1.0L;
    sv.lat = asinl(v.z);
    if ((sv.lat == M_PI_2) || (sv.lat == -M_PI_2))
      sv.lon = 0.0L;
    else
      sv.lon = atan2l(v.y, v.x);
    return sv;
  } else {
...
*/
{
  long double sd;
  sd =
      cosl(M_PI_2 - ll1.lat) * cosl(M_PI_2 - ll2.lat) +
...
*/
{
  ...
  mindist = 5;
  l1[0] = tri->verts[0].lat;
  l1[1] = tri->verts[0].lon;
  l2[0] = tri->verts[1].lat;
  l2[1] = tri->verts[1].lon;
  l3[0] = tri->verts[2].lat;
  l3[1] = tri->verts[2].lon;
  tri->edges[0] =
      acosl(cosl(M_PI_2 - l2[0]) * cosl(M_PI_2 - l3[0]) +
            sinl(M_PI_2 - l2[0]) * sinl(M_PI_2 - l3[0]) * cosl(l2[1] - l3[1]));
  tri->edges[1] =
      acosl(cosl(M_PI_2 - l1[0]) * cosl(M_PI_2 - l3[0]) +
            sinl(M_PI_2 - l1[0]) * sinl(M_PI_2 - l3[0]) * cosl(l1[1] - l3[1]));
  tri->edges[2] =
      acosl(cosl(M_PI_2 - l2[0]) * cosl(M_PI_2 - l1[0]) +
            sinl(M_PI_2 - l2[0]) * sinl(M_PI_2 - l1[0]) * cosl(l2[1] - l1[1]));
  tri->angles[0] =
      acosl((cosl(tri->edges[0]) - cosl(tri->edges[1]) * cosl(tri->edges[2])) /
            (sinl(tri->edges[1]) * sinl(tri->edges[2])));
  tri->angles[1] =
      acosl((cosl(tri->edges[1]) - cosl(tri->edges[0]) * cosl(tri->edges[2])) /
            (sinl(tri->edges[0]) * sinl(tri->edges[2])));
  tri->angles[2] =
      acosl((cosl(tri->edges[2]) - cosl(tri->edges[0]) * cosl(tri->edges[1])) /
            (sinl(tri->edges[0]) * sinl(tri->edges[1])));
  for (i = 0; i < 3; i++)
    tri->edges[i] = tri->edges[i] * DgGeoSphRF::earthRadiusKM();
  if (tri->edges[0] < mindist) {
    p = (tri->edges[0] + tri->edges[1] + tri->edges[2]) / 2;
    tri->area = sqrtl(p * (p - tri->edges[0]) * (p - tri->edges[1]) *
                      (p - tri->edges[2]));
  } else
    tri->area = (tri->angles[0] + tri->angles[1] + tri->angles[2] - M_PI) *
...
*/
{
  ...
  az = atan2l(cosl(pt2.lat) * sinl(pt2.lon - pt1.lon),
              cosl(pt1.lat) * sinl(pt2.lat) -
...
{
  GeoCoord pt;
  Vec3D nn1, nn2, p11, p12, p21, p22, pp, pp2;
  ...
  if ((nn2.z * nn1.y - nn1.z * nn2.y) != 0.0L) {
    ...
    pp.z = b * pp.x;
             ...
             ((nn1.x * nn2.y - nn2.x * nn1.y) == 0.0L) &&
             ((nn1.x * nn2.z - nn2.x * nn1.z) == 0.0L)) {
  ...
  if (sign == 0) {
    ...
    if ((pt.lon <= maxlon) && (pt.lon >= minlon))
      ...
      if ((pt.lon <= maxlon) && (pt.lat >= minlon))
        ...
        dgcerr << "Error of GCintersect: the point is not on great circle "
                  "segment.\n";
        pt.lat = UNDEFVAL;
        pt.lon = UNDEFVAL;
        return pt;
...
*/
{
  ...
  if ((fabsl(az) < PRECISION) || (fabsl(fabsl(az) - M_PI) < PRECISION)) {
    if (fabsl(az) < PRECISION)
      pt2.lat = pt.lat + distance;
    else
      pt2.lat = pt.lat - distance;
    pt2.lon = pt.lon;
    ...
    sinlat = sinl(pt.lat) * cosl(distance) +
             cosl(pt.lat) * sinl(distance) * cosl(az);
    if (sinlat > 1.0L)
      sinlat = 1.0L;
    if (sinlat < -1.0L)
      sinlat = -1.0L;
    pt2.lat = asinl(sinlat);
    if ((pt2.lat == M_PI_2) || (pt2.lat == -M_PI_2))
      pt2.lon = 0.0L;
    else {
      sinlon = sinl(az) * sinl(distance) / cosl(pt2.lat);
      coslon = (cosl(distance) - sinl(pt.lat) * sinl(pt2.lat)) / cosl(pt.lat) /
               cosl(pt2.lat);
      if (sinlon > 1.0L)
        sinlon = 1.0L;
      if (sinlon < -1.0L)
