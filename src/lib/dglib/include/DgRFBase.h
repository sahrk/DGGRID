...
class DgRFBase {
   ...
   public:
      ...
      DgLocation* convert (DgLocation *const loc) const; // converts in place

      DgLocVector& convert (DgLocVector& vec) const; // converts in place

      void convert (DgLocBase *const loc) const // converts in place
                { loc->convertTo(*this); }
      ...
      virtual std::unique_ptr<DgLocation> vecLocation (const DgDVec2D& v) const
                    { return {}; /*Returns an empty unique pointer*/ }
   ...
   protected:
      ...
      DgRFBase (DgRFNetwork& networkIn, const string& nameIn)
         : network_ (&networkIn), name_ (nameIn), precision_ (6),
           connectTo_ (nullptr), connectFrom_ (nullptr)
      ...
      virtual std::unique_ptr<DgLocation> buildLocation (DgAddressBase* addIn) const;
                             // does not make copy
