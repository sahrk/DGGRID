...
public:
  ...
  static DgGeoCoord gcIntersect(const DgGeoCoord &g11, const DgGeoCoord &g12,
  ...
  DgGeoCoord(long double lon, long double lat, bool rads = true) {
    if (rads)
      *this = DgDVec2D(lon, lat);
    else
      *this = DgDVec2D(lon * M_PI_180, lat * M_PI_180);
  }

  DgGeoCoord(const GeoCoord &coord) { *this = DgDVec2D(coord.lon, coord.lat); }

  DgGeoCoord &operator=(const DgGeoCoord &coord) {
    DgDVec2D::operator=(coord);
    return *this;
  }
  ...
  void normalize(void) {
    if (fabsl(latDegs()) >= (90.0L - tolerance)) {
      ...
      while (lonDegs() <= -180.0L)
        setLonDeg(lonDegs() + 360.0L);
      while (lonDegs() > 180.0L)
        setLonDeg(lonDegs() - 360.0L);
    }
  }
...
public:
  ...
  */
  ...
  virtual DgDVec2D getVecLocation(const DgLocation &loc) const {
    return DgDVec2D(getAddress(loc)->lonDegs(), getAddress(loc)->latDegs());
  }
  ...
  virtual string add2str(const DgGeoCoord &add) const { return string(add); }
  ...
  virtual long double dist2dbl(const long double &dist) const { return dist; }
...
protected:
  DgEllipsoidRF(DgRFNetwork &networkIn, const string &nameIn)
      : DgGeoDatumRF<DgGeoCoord, long double>(networkIn, nameIn), a_(-1.0L),
        b_(-1.0L), f_(-1.0L), e_(-1.0L), es_(-1.0L), as_(-1.0L), bs_(-1.0L),
        ra_(-1.0L), one_es_(-1.0L), rone_es_(-1.0L) {}

  DgEllipsoidRF(DgRFNetwork &networkIn, const string &nameIn,
                const DgEllipsoidRF &ell)
      : DgGeoDatumRF<DgGeoCoord, long double>(networkIn, nameIn), a_(ell.a()),
        b_(ell.b()), f_(ell.f()), e_(ell.e()), es_(ell.es()), as_(ell.as()),
        bs_(ell.bs()), ra_(ell.ra()), one_es_(ell.one_es()),
        rone_es_(ell.rone_es()) {
    undefLoc_ = makeLocation(undefAddress());
  }

  DgEllipsoidRF(DgRFNetwork &networkIn, const string &nameIn, long double aIn,
                long double bIn)
      : DgGeoDatumRF<DgGeoCoord, long double>(networkIn, nameIn), a_(aIn),
        b_(bIn) {
