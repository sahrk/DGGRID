...
Each generated orientation file is a DGGRID metafile for one placement and can be used as input to reproduce that placement. Its vertex coordinates use `output_geographic_mode`; the file sets `input_geographic_mode` to the same value so the orientation is read back correctly. Keep any referenced relative input files available when replaying it.
...
**6. Choosing geographic input and output coordinates:** `input_geographic_mode` and `output_geographic_mode` independently select the meaning of longitude/latitude coordinates. Each defaults to `SPHERE`, preserving the historical spherical behavior. `WGS84` interprets latitude as geodetic latitude on the WGS 84 ellipsoid. At input, DGGRID converts it to authalic latitude on the WGS 84 authalic sphere before using the selected projection. At output, DGGRID converts authalic latitude back to WGS 84 geodetic latitude. Longitude is unchanged.

| Input mode | Output mode | Geographic input | Geographic output |
|------------|-------------|------------------|-------------------|
| `SPHERE` | `SPHERE` | Spherical longitude/authalic latitude | Spherical longitude/authalic latitude |
| `WGS84` | `SPHERE` | WGS 84 longitude/geodetic latitude | Spherical longitude/authalic latitude |
| `SPHERE` | `WGS84` | Spherical longitude/authalic latitude | WGS 84 longitude/geodetic latitude |
| `WGS84` | `WGS84` | WGS 84 longitude/geodetic latitude | WGS 84 longitude/geodetic latitude |

If either mode is `WGS84`, `proj_datum` must be `WGS84_AUTHALIC_SPHERE`. The `WGS84_MEAN_SPHERE` and `CUSTOM_SPHERE` values remain available when both modes are `SPHERE`. `proj_datum` still selects the radius of the *internal sphere*; it does not select an ellipsoidal projection. ISEA and Fuller both operate on that sphere. The authalic conversion preserves area between the WGS 84 ellipsoid and its authalic sphere, so ISEA remains equal area on the ellipsoid. Fuller does not become equal area.

The input mode applies to geographic point and polygon files, `GEO` addresses, and user supplied geographic positions such as `dggs_vert0_lon`/`dggs_vert0_lat` and `region_center_lon`/`region_center_lat`. A user supplied orientation longitude or latitude makes the complete longitude/latitude pair use the input mode; the unspecified component takes its current default or preset value. Built in and preset orientations remain spherical until a user supplies one of those components. Random placement is generated on the internal sphere. Azimuths, projected coordinates, grid addresses, and radius values are not latitude values and are not converted.

Geographic clipping vertices use the input mode. Subsequent clipping and edge densification operate on the internal sphere; selecting `WGS84` does not make polygon edges follow WGS 84 ellipsoidal geodesics. The output mode applies to `GEO` addresses and generated geographic centers, boundaries, random points, and other geographic geometries. Generated orientation metafiles express their latitude in the output mode and set their input mode accordingly so that replay uses the same placement.
...
input_geographic_mode WGS84
output_geographic_mode SPHERE
```

To read spherical coordinates and write WGS 84 coordinates, exchange the two mode values. A fixed cell address is independent of the input mode; only the output mode controls its geographic geometry. Conversely, when converting a geographic point to a cell address, only the input mode affects its cell assignment.

WGS 84 output coordinates are written in longitude, latitude order. Shapefile and GDAL output use a WGS 84 geographic CRS (EPSG:4326, with traditional GIS coordinate order where applicable); GeoJSON uses the RFC 7946 longitude, latitude convention, and KML uses WGS 84 longitude, latitude. In `SPHERE` output mode, coordinates retain their historical spherical meaning. GeoJSON and KML normally imply WGS 84 to readers, so their spherical mode files should be interpreted with that DGGRID setting in mind; spherical coordinates must not be described as EPSG:4326. Text and AIGen coordinates have no embedded CRS declaration.
...
All DGG cell boundaries and center points output from **DGGRID** are longitude/latitude coordinates in decimal degrees. Their latitude model is selected by `output_geographic_mode` (see **Section 4.6**).
...
| **input_geographic_mode** *(choice)* | Latitude model for geographic input | SPHERE, WGS84 | SPHERE | WGS84 requires `proj_datum WGS84_AUTHALIC_SPHERE`; see Section 4.6 | Geographic input, `GEO` addresses, and user supplied geographic placement |
...
| **output_geographic_mode** *(choice)* | Latitude model for geographic output | SPHERE, WGS84 | SPHERE | WGS84 requires `proj_datum WGS84_AUTHALIC_SPHERE`; see Section 4.6 | `GEO` addresses and geographic geometry |
...
in decimal degrees. The parameters input_address_type and output_address_type refer to this address form as GEO. The latitude is spherical/authalic or WGS 84 geodetic according to the corresponding geographic mode.
