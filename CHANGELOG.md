...
### Changed
- icosahedron and ISEA constants are now their exact closed forms, correctly rounded to 40+ digit long double literals in DgConstants.h: the default dggs_vert0_lat is atan(phi) = 58.282525588538995 (was 58.28252559; this puts the poles exactly on icosahedron edge midpoints), the vertex ring latitude is atan(1/2) (was 26.565051177 deg), and Snyder's g, R'/R and the planar origin offsets are exact (were 10 digits). The WGS84 mean and authalic sphere radii are computed exactly from a = 6378137 m and 1/f = 298.257223563. The icosahedron/ISEA code uses long double pi constants instead of the double M_PI. ISEA and Fuller geographic output changes at the <= 1e-8 degree level (as arc length; longitudes within ~4 degrees of a pole by up to 3e-8), and a few edge-of-region cells in COARSE_CELLS clipping change acceptance.
### Fixed
- Class III aperture 7 hexagon grids (ISEA7H, IGEO7 and FULLER7H at odd resolutions 1, 3, 5, ..., and any aperture sequence with an odd number of aperture 7 steps) placed some vertices of the cells that straddle certain icosahedron face edges on the wrong face, so the projection inverse extrapolated them: ISEA vertices were off by up to ~11 km at resolution 1, ~5.5 km at resolution 3 and ~1.4 km at resolution 5 (Fuller by far more), the cell boundaries did not meet their neighbours' and the cell areas did not sum to the sphere. 20 cells at resolution 1, 140 at resolution 3 and 980 at resolution 5 were affected. Points in the planar quad frame past a quad's far edge are now re-expressed in the neighbouring quad before the face is chosen, so these cells are now watertight and their vertices exact. Only cell boundary (vertex) output changes; cell ownership, indexing, centres, neighbors and children are unchanged. IVEA7H output is unchanged, since IVEA already carried such points into the neighbouring face. Class I and II grids, pentagons, triangle and diamond grids are not affected.
- the dggs_type preset parser treated any preset name not starting with ISEA as a FULLER grid; it now matches the projection prefix from a table and reports a fatal error if none matches.
